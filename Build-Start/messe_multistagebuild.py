...
for x in range(1,21):
   ...
   command ="sshpass -p '123456' ssh udoo@udoo 'cd /home/udoo/go_api; docker build -f Dockerfile.multistage . -t go_server_multi'" 
   ...
   #########################
   command ="sshpass -p '123456' ssh udoo@udoo 'docker rmi go_server_multi'"
   status, out=commands.getstatusoutput(command)

   time.sleep(1)
      
   command ="sshpass -p '123456' ssh udoo@udoo 'docker image rm $(docker image list -f \"dangling=true\" -qa)'"
   status, out=commands.getstatusoutput(command)
